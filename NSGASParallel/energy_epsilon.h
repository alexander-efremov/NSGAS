...
inline double energy_A(double Gamma, double* sigma_k1, double* e_k)
{
	...
	for (i = 1; i < qq; i++)
	{
		...
		{
			a = i * M + j;
			A[a][0] = Gamma / (2 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[(i - 1) * M + j]) - (Mu(Gamma, e_k[(i - 1) * M + j]) + Mu(Gamma, e_k[i * M + j])));
			A[a][1] = Gamma / (2 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[i * M + j - 1]) - (Mu(Gamma, e_k[i * M + j - 1]) + Mu(Gamma, e_k[i * M + j])));
			A[a][2] = sigma_k1[a] * sigma_k1[a] / tau - Gamma / (2 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (2 * e_k[i * M + j] - e_k[(i + 1) * M + j] - e_k[(i - 1) * M + j]) -
				Gamma / (2 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (2 * e_k[i * M + j] - e_k[i * M + j + 1] - e_k[i * M + j - 1]) +
				Gamma / (2 * hx * hx * Pr * Re) * (2 * Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[(i + 1) * M + j]) + Mu(Gamma, e_k[(i - 1) * M + j])) +
				Gamma / (2 * hy * hy * Pr * Re) * (2 * Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[i * M + j + 1]) + Mu(Gamma, e_k[i * M + j - 1]));
			A[a][3] = -Gamma / (2 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j + 1] - e_k[i * M + j]) + (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[i * M + j + 1])));
			A[a][4] = -Gamma / (2 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[(i + 1) * M + j] - e_k[i * M + j]) + (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[(i + 1) * M + j])));
		}
	}
	...
	for (i = qq; i < qq + w - 1; i++)
	{
		...
		A[a][0] = Gamma / (2 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[(i - 1) * M + j]) - (Mu(Gamma, e_k[(i - 1) * M + j]) + Mu(Gamma, e_k[i * M + j])));
		A[a][1] = Gamma / (2 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[i * M + j - 1])) - Gamma / (4 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j - 1]) + Mu(Gamma, e_k[i * M + j])) - Gamma / (8 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j - 1]) + 2 * Mu(Gamma, e_k[i * M + j]));
		A[a][2] = sigma_k1[a] * sigma_k1[a] / tau - Gamma / (2 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (2 * e_k[i * M + j] - e_k[(i + 1) * M + j] - e_k[(i - 1) * M + j]) -
			Gamma / (2 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (2 * e_k[i * M + j] - e_k[i * M + j + 1] - e_k[i * M + j - 1]) +
			Gamma / (8 * hx * hx * Pr * Re) * (8 * Mu(Gamma, e_k[i * M + j]) + 3 * Mu(Gamma, e_k[(i + 1) * M + j]) + 4 * Mu(Gamma, e_k[(i - 1) * M + j])) +
			Gamma / (8 * hy * hy * Pr * Re) * (8 * Mu(Gamma, e_k[i * M + j]) + 4 * Mu(Gamma, e_k[i * M + j + 1]) + 3 * Mu(Gamma, e_k[i * M + j - 1]));
		A[a][3] = -Gamma / (2 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j + 1] - e_k[i * M + j]) + (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[i * M + j + 1])));
		A[a][4] = -Gamma / (2 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[(i + 1) * M + j] - e_k[i * M + j])) - Gamma / (4 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[(i + 1) * M + j])) - Gamma / (8 * hx * hx * Pr * Re) * (2 * Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[(i + 1) * M + j]));
	}
	...
	for (i = qq; i < qq + w - 1; i++)
	{
		...
		A[a][2] = sigma_k1[a] * sigma_k1[a] / tau - Gamma / (2 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (2 * e_k[i * M + j] - e_k[(i + 1) * M + j] - e_k[(i - 1) * M + j]) -
			Gamma / (2 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (2 * e_k[i * M + j] - e_k[i * M + j + 1] - e_k[i * M + j - 1]) +
			Gamma / (8 * hx * hx * Pr * Re) * (8 * Mu(Gamma, e_k[i * M + j]) + 3 * Mu(Gamma, e_k[(i + 1) * M + j]) + 4 * Mu(Gamma, e_k[(i - 1) * M + j])) +
			Gamma / (8 * hy * hy * Pr * Re) * (8 * Mu(Gamma, e_k[i * M + j]) + 3 * Mu(Gamma, e_k[i * M + j + 1]) + 4 * Mu(Gamma, e_k[i * M + j - 1]));
		A[a][3] = -Gamma / (2 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j + 1] - e_k[i * M + j])) - Gamma / (4 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[i * M + j + 1])) - Gamma / (8 * hy * hy * Pr * Re) * (2 * Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[i * M + j + 1]));
		A[a][4] = -Gamma / (2 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[(i + 1) * M + j] - e_k[i * M + j])) - Gamma / (4 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[(i + 1) * M + j])) - Gamma / (8 * hx * hx * Pr * Re) * (2 * Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[(i + 1) * M + j]));
	}
	...
	for (j = cntr - q + 2; j < (cntr + q - 1); j++)
	{
		...
		A[a][1] = Gamma / (4 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[i * M + j - 1]) - (Mu(Gamma, e_k[i * M + j - 1]) + Mu(Gamma, e_k[i * M + j])));
		A[a][2] = sigma_k1[a] * sigma_k1[a] / (2 * tau) - Gamma / (4 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (2 * e_k[i * M + j] - 2 * e_k[(i + 1) * M + j]) -
			Gamma / (4 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (2 * e_k[i * M + j] - e_k[i * M + j + 1] - e_k[i * M + j - 1]) +
			Gamma / (4 * hx * hx * Pr * Re) * (2 * Mu(Gamma, e_k[i * M + j]) + 2 * Mu(Gamma, e_k[(i + 1) * M + j])) +
			Gamma / (4 * hy * hy * Pr * Re) * (2 * Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[i * M + j - 1]) + Mu(Gamma, e_k[i * M + j + 1]));
		A[a][3] = -Gamma / (4 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j + 1] - e_k[i * M + j]) + (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[i * M + j + 1])));
		A[a][4] = -Gamma / (2 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[(i + 1) * M + j] - e_k[i * M + j]) + (Mu(Gamma, e_k[(i + 1) * M + j]) + Mu(Gamma, e_k[i * M + j])));
	}													  
	...
	for (i = qq + 1; i < qq + w - 1; i++)
	{
		...
		A[a][0] = Gamma / (4 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[(i - 1) * M + j]) + Gamma / (8 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[(i - 1) * M + j])
			- Gamma / (4 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[(i - 1) * M + j])) - Gamma / (8 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) + 2 * Mu(Gamma, e_k[(i - 1) * M + j]));
		A[a][2] = sigma_k1[a] * sigma_k1[a] * (1 / (4 * tau) + 1 / (4 * tau)) - Gamma / (8 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (4 * e_k[i * M + j] - 3 * e_k[(i - 1) * M + j]) -
			Gamma / (8 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (4 * e_k[i * M + j] - 3 * e_k[i * M + j + 1]) +
			Gamma / (8 * hx * hx * Pr * Re) * (4 * Mu(Gamma, e_k[i * M + j]) + 4 * Mu(Gamma, e_k[(i - 1) * M + j])) +
			Gamma / (8 * hy * hy * Pr * Re) * (4 * Mu(Gamma, e_k[i * M + j]) + 4 * Mu(Gamma, e_k[i * M + j + 1]));
		A[a][3] = -Gamma / (4 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j + 1] - e_k[i * M + j]) - Gamma / (8 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j + 1] - e_k[i * M + j])
			- Gamma / (4 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j + 1]) + Mu(Gamma, e_k[i * M + j])) - Gamma / (8 * hy * hy * Pr * Re) * (2 * Mu(Gamma, e_k[i * M + j + 1]) + Mu(Gamma, e_k[i * M + j]));
		A[a][5] = -Gamma / (2 * tg * hx * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]);
		A[a][8] = Gamma / (2 * tg * hx * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]);
	}
	...
	for (i = qq + 1; i < qq + w - 1; i++)
	{
		...
		A[a][0] = Gamma / (4 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[(i - 1) * M + j]) + Gamma / (8 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[(i - 1) * M + j])
			- Gamma / (4 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[(i - 1) * M + j])) - Gamma / (8 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) + 2 * Mu(Gamma, e_k[(i - 1) * M + j]));
		A[a][1] = Gamma / (4 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[(i) * M + j - 1]) + Gamma / (8 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[(i) * M + j - 1])
			- Gamma / (4 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[(i) * M + j - 1])) - Gamma / (8 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) + 2 * Mu(Gamma, e_k[(i) * M + j - 1]));
		A[a][2] = sigma_k1[a] * sigma_k1[a] * (1 / (4 * tau) + 1 / (4 * tau)) - Gamma / (8 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (4 * e_k[i * M + j] - 3 * e_k[(i - 1) * M + j]) -
			Gamma / (8 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (4 * e_k[i * M + j] - 3 * e_k[i * M + j - 1]) +
			Gamma / (8 * hx * hx * Pr * Re) * (4 * Mu(Gamma, e_k[i * M + j]) + 4 * Mu(Gamma, e_k[(i - 1) * M + j])) +
			Gamma / (8 * hy * hy * Pr * Re) * (4 * Mu(Gamma, e_k[i * M + j]) + 4 * Mu(Gamma, e_k[i * M + j - 1]));
		A[a][6] = -Gamma / (2 * tg * hx * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]);
		A[a][7] = Gamma / (2 * tg * hx * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]);
	}
	...
	A[a][0] = Gamma / (4 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[(i - 1) * M + j]) + Gamma / (8 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[(i - 1) * M + j])
		- Gamma / (4 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[(i - 1) * M + j]) + Mu(Gamma, e_k[i * M + j])) - Gamma / (8 * hx * hx * Pr * Re) * (2 * Mu(Gamma, e_k[(i - 1) * M + j]) + Mu(Gamma, e_k[i * M + j]));
	A[a][1] = Gamma / (4 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[i * M + j - 1]) - (Mu(Gamma, e_k[i * M + j - 1]) + Mu(Gamma, e_k[i * M + j])));
	A[a][2] = sigma_k1[a] * sigma_k1[a] * (3 / (4 * tau) + 1 / (8 * tau)) - Gamma / (8 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (7 * e_k[i * M + j] - 4 * e_k[(i + 1) * M + j] - 3 * e_k[(i - 1) * M + j]) -
		Gamma / (8 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (7 * e_k[i * M + j] - 4 * e_k[i * M + j + 1] - 2 * e_k[i * M + j - 1]) +
		Gamma / (8 * hx * hx * Pr * Re) * (7 * Mu(Gamma, e_k[i * M + j]) + 4 * Mu(Gamma, e_k[(i + 1) * M + j]) + 4 * Mu(Gamma, e_k[(i - 1) * M + j])) +
		Gamma / (8 * hy * hy * Pr * Re) * (7 * Mu(Gamma, e_k[i * M + j]) + 4 * Mu(Gamma, e_k[i * M + j + 1]) + 2 * Mu(Gamma, e_k[i * M + j - 1])) + Gamma / (2 * tg * hx * hy * Pr * Re) * Mu(Gamma, e_k[(i) * M + j]);
	A[a][3] = -Gamma / (2 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j + 1] - e_k[i * M + j]) + (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[i * M + j + 1])));
	A[a][4] = -Gamma / (2 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[(i + 1) * M + j] - e_k[i * M + j]) + (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[(i + 1) * M + j])));
	A[a][5] = -Gamma / (2 * tg * hx * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]);
	...
	A[a][1] = Gamma / (2 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[i * M + j - 1]) - (Mu(Gamma, e_k[i * M + j - 1]) + Mu(Gamma, e_k[i * M + j])));
	A[a][2] = sigma_k1[a] * sigma_k1[a] * (3 / (4 * tau) + 1 / (8 * tau)) - Gamma / (8 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (7 * e_k[i * M + j] - 4 * e_k[(i + 1) * M + j] - 3 * e_k[(i - 1) * M + j]) -
		Gamma / (8 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (7 * e_k[i * M + j] - 2 * e_k[i * M + j + 1] - 4 * e_k[i * M + j - 1]) +
		Gamma / (8 * hx * hx * Pr * Re) * (7 * Mu(Gamma, e_k[i * M + j]) + 4 * Mu(Gamma, e_k[(i + 1) * M + j]) + 4 * Mu(Gamma, e_k[(i - 1) * M + j])) +
		Gamma / (8 * hy * hy * Pr * Re) * (7 * Mu(Gamma, e_k[i * M + j]) + 2 * Mu(Gamma, e_k[i * M + j + 1]) + 4 * Mu(Gamma, e_k[i * M + j - 1])) + Gamma / (2 * tg * hx * hy * Pr * Re) * Mu(Gamma, e_k[(i) * M + j]);
	A[a][3] = -Gamma / (4 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j + 1] - e_k[i * M + j]) + (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[i * M + j + 1])));
	A[a][4] = -Gamma / (2 * hx * hx * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[(i + 1) * M + j] - e_k[i * M + j]) + (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[(i + 1) * M + j])));
	A[a][6] = -Gamma / (2 * tg * hx * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]);
	...
	A[a][1] = Gamma / (4 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[i * M + j - 1]) + Gamma / (8 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j] - e_k[i * M + j - 1])
		- Gamma / (4 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j - 1]) + Mu(Gamma, e_k[i * M + j])) - Gamma / (8 * hy * hy * Pr * Re) * (2 * Mu(Gamma, e_k[i * M + j - 1]) + Mu(Gamma, e_k[i * M + j]));
	A[a][2] = sigma_k1[a] * sigma_k1[a] * (1 / (4 * tau) + 1 / (2 * tau)) - Gamma / (8 * hx * hx * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (6 * e_k[i * M + j] - 4 * e_k[(i - 1) * M + j]) -
		Gamma / (8 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (6 * e_k[i * M + j] - 3 * e_k[i * M + j + 1] - 3 * e_k[i * M + j - 1]) +
		Gamma / (8 * hx * hx * Pr * Re) * (6 * Mu(Gamma, e_k[i * M + j]) + 4 * Mu(Gamma, e_k[(i - 1) * M + j])) +
		Gamma / (8 * hy * hy * Pr * Re) * (6 * Mu(Gamma, e_k[i * M + j]) + 4 * Mu(Gamma, e_k[i * M + j + 1]) + 4 * Mu(Gamma, e_k[i * M + j - 1])) - Gamma / (tg * hx * hy * Pr * Re) * Mu(Gamma, e_k[(i) * M + j]);
	A[a][3] = -Gamma / (4 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j + 1] - e_k[i * M + j]) - Gamma / (8 * hy * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]) / e_k[i * M + j] * (e_k[i * M + j + 1] - e_k[i * M + j])
		- Gamma / (4 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) + Mu(Gamma, e_k[i * M + j + 1])) - Gamma / (8 * hy * hy * Pr * Re) * (Mu(Gamma, e_k[i * M + j]) + 2 * Mu(Gamma, e_k[i * M + j + 1]));
	A[a][7] = Gamma / (2 * tg * hx * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]);
	A[a][8] = Gamma / (2 * tg * hx * hy * Pr * Re) * Mu(Gamma, e_k[i * M + j]);
...
inline double energy(double gamma, double* sigma_k1, double* sigma_k, double* u_k, double* v_k, double* u_k1, double* v_k1, double* e2, double* e_k, double* e_k1)
{
